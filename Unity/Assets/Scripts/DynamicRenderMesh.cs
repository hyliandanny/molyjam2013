...
public class DynamicRenderMesh : DynamicMesh {
	...
	public void Generate(Vector2[] lowerCurve,Vector2[] upperCurve) {
		int segments = lowerCurve.Length-1;
		Vector3[] vertices = new Vector3[4*segments];
		Vector2[] uvs = new Vector2[4*segments];
		int[] tris = new int[3*vertices.Length];
 
		for(int i = 0; i < segments; i++) {
			vertices[i*4]   = new Vector3(((Vector2)lowerCurve[i]).x,   ((Vector2)lowerCurve[i]).y, depth);
			vertices[1+i*4] = new Vector3(((Vector2)lowerCurve[i]).x,   ((Vector2)upperCurve[i]).y, depth);
			vertices[2+i*4] = new Vector3(((Vector2)lowerCurve[i+1]).x, ((Vector2)lowerCurve[i]).y, depth);
			vertices[3+i*4] = new Vector3(((Vector2)lowerCurve[i+1]).x, ((Vector2)upperCurve[i]).y, depth);
			
			uvs[i*4]   = new Vector2(((Vector2)lowerCurve[i]).x,   0);
			uvs[1+i*4] = new Vector2(((Vector2)lowerCurve[i]).x,   1);
			uvs[2+i*4] = new Vector2(((Vector2)lowerCurve[i+1]).x, 0);
			uvs[3+i*4] = new Vector2(((Vector2)lowerCurve[i+1]).x, 1);
